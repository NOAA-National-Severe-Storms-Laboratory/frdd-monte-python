...
parser.add_option("-e", dest="nt", type="int", help = "Number of timesteps")
parser.add_option("-t", dest="t", type="int", help = "Timestep to process")

(options, args) = parser.parse_args()

if ((options.exp_dir == None) or (options.nt == None) or (options.t == None)):
    print()
    parser.print_help()
    print()
    sys.exit(1)
...
for f, file in enumerate(files):
   if (file[0] == '2'):  #only process summary files
...
pmm_dz[:,:] = prob_match_mean(dz[:,:,:], mean_dz[:,:], neighborhood)
...
fout.variables['TIME'][t] = time
fout.variables['PMM_DZ'][t,:,:] = pmm_dz
