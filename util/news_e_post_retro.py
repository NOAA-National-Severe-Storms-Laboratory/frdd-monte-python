...
parser.add_option("-t", dest="t", type="int", help = "Timesteps to process")

(options, args) = parser.parse_args()

if ((options.dir == None) or (options.outdir == None) or (options.fcst_nt == None) or (options.t == None)):
   print()
   parser.print_help()
   print()
   sys.exit(1)
...
####################################### Find Member dirs and wrfout files: ######################################################
...
dbz_2km = np.zeros((ny,nx))           	        	#Simulated reflectivity (dBZ) at 1 km AGL     
dbz_composite = np.zeros((ny,nx))                   	#Simulated reflectivity (dBZ) averaged between 0 and 7000 m AGL     
dbz_col_max = np.zeros((ny,nx))                   	#Simulated column max reflectivity (dBZ)     
...
temp_path = os.path.join(dir, member_files[t])
...
############################### Convert time to seconds - only handles 00z shift through ~6 UTC  ##################################

time_in_seconds = hour * 3600. + min * 60. + sec
if (time_in_seconds < 30000.):
...
bunk_r_u, bunk_r_v, bunk_l_u, bunk_l_v = calc_bunkers(p, z_agl, dz, uc, vc)
...
dbz_col_max = np.max(dbz, axis=0)

temp = np.zeros((dbz.shape[1], dbz.shape[2])) + 2000.
dbz_upper, dbz_lower, dbz_interp = find_upper_lower(temp, z_agl)         
dbz_2km = calc_interp(dbz, dbz_upper, dbz_lower, dbz_interp)
...
if (t == 0):
   ...
   fout.createVariable('DZ_MAX', 'f4', ('NT','NY','NX',))
   fout.createVariable('DZ_2KM', 'f4', ('NT','NY','NX',))
...
fout.variables['TIME'][t] = time_in_seconds
   
fout.variables['U_10'][t,:,:] = u_10
fout.variables['V_10'][t,:,:] = v_10
fout.variables['WS_MAX_10'][t,:,:] = ws_max_10
fout.variables['P_SFC'][t,:,:] = p_sfc
fout.variables['T_2'][t,:,:] = t_2
fout.variables['TD_2'][t,:,:] = td_2
fout.variables['TH_2'][t,:,:] = th_2
fout.variables['QV_2'][t,:,:] = qv_2

fout.variables['HAIL'][t,:,:] = hailnc
fout.variables['GRAUPEL'][t,:,:] = graupelnc
fout.variables['SNOW'][t,:,:] = snownc
fout.variables['RAIN'][t,:,:] = rainnc

fout.variables['GRAUPEL_MAX'][t,:,:] = graupel_max

fout.variables['T_ML'][t,:,:] = t_75mb
fout.variables['TH_ML'][t,:,:] = th_75mb
fout.variables['TD_ML'][t,:,:] = td_75mb
fout.variables['TH_V_ML'][t,:,:] = th_v_75mb
fout.variables['TH_E_ML'][t,:,:] = th_e_75mb
fout.variables['QV_ML'][t,:,:] = qv_75mb
fout.variables['P_ML'][t,:,:] = p_75mb

fout.variables['LCL_ML'][t,:,:] = lcl_75mb
fout.variables['LFC_ML'][t,:,:] = lfc_75mb
fout.variables['EL_ML'][t,:,:] = el_75mb
fout.variables['CAPE_ML'][t,:,:] = cape_75mb
fout.variables['CIN_ML'][t,:,:] = cin_75mb
fout.variables['CAPE_0TO3_ML'][t,:,:] = cape_0to3_75mb

fout.variables['U_500'][t,:,:] = u_500
fout.variables['V_500'][t,:,:] = v_500
fout.variables['SHEAR_U_0TO1'][t,:,:] = shear_u_0to1
fout.variables['SHEAR_V_0TO1'][t,:,:] = shear_v_0to1
fout.variables['SHEAR_U_0TO6'][t,:,:] = shear_u_0to6
fout.variables['SHEAR_V_0TO6'][t,:,:] = shear_v_0to6
fout.variables['BUNK_R_U'][t,:,:] = bunk_r_u
fout.variables['BUNK_R_V'][t,:,:] = bunk_r_v
fout.variables['BUNK_L_U'][t,:,:] = bunk_l_u
fout.variables['BUNK_L_V'][t,:,:] = bunk_l_v
fout.variables['SRH_0TO1'][t,:,:] = srh_0to1
fout.variables['SRH_0TO3'][t,:,:] = srh_0to3
fout.variables['SRH_0TO6'][t,:,:] = srh_0to6
fout.variables['STP_ML'][t,:,:] = stp_75mb

fout.variables['DZ_MAX'][t,:,:] = dbz_col_max
fout.variables['DZ_2KM'][t,:,:] = dbz_2km
fout.variables['DZ_COMP'][t,:,:] = dbz_composite
fout.variables['WZ_0TO2'][t,:,:] = wz_0to2
fout.variables['WZ_2TO5'][t,:,:] = wz_2to5
fout.variables['WZ_0TO6'][t,:,:] = wz_0to6
fout.variables['UH_0TO2'][t,:,:] = uh_0to2
fout.variables['UH_2TO5'][t,:,:] = uh_2to5
fout.variables['UH_0TO6'][t,:,:] = uh_0to6

fout.variables['W_UP_MAX'][t,:,:] = w_up_max
fout.variables['W_DN_MAX'][t,:,:] = w_down_max
fout.variables['W_1KM'][t,:,:] = w_1km
