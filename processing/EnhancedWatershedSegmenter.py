...
class EnhancedWatershed(object):
    ...
    """
    def __init__(self, min_thresh, data_increment, max_thresh, size_threshold_pixels, delta, local_max_area):
        self.min_thresh = min_thresh
        self.data_increment = data_increment
        self.max_thresh = max_thresh
        self.max_size = size_threshold_pixels
        self.local_max_area = local_max_area
        self.delta = delta
    ...
    def label(self, input_grid):
        ...
        """
        marked, orig = self.find_local_maxima(input_grid)
    ...
    def find_local_maxima(self, input_grid):
        ...
        centers = OrderedDict()
        for p in list(pixels.keys()):
            centers[p] = []
        marked = np.ones(q_data.shape, dtype=int) * self.UNMARKED
        MIN_INFL = int(np.round(1 + 0.5 * np.sqrt(self.local_max_area)))
        MAX_INFL = 2 * MIN_INFL
        marked_so_far = []
        ...
        for b in sorted(list(pixels.keys()),reverse=True):
        ...
        original_marked = np.copy(marked)
        ...
        for delta in range(0, self.delta + 1):
            ...
            for b in sorted(list(centers.keys()), reverse=True):
                bin_lower = b - delta
    ...
    def is_closest(point, center, centers, bin_num):
        ...
        for o_bin in range(bin_thresh, len(list(centers.keys()))):
